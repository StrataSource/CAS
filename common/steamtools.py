...
def get_steam_path() -> Path:
    if sys.platform == 'win32':
        steam_key = winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, r'SOFTWARE\WOW6432Node\Valve\Steam', 0, winreg.KEY_READ)
        steam_path = winreg.QueryValueEx(steam_key, 'InstallPath')
        winreg.CloseKey(steam_key)

        return Path(steam_path[0]).resolve()
    else:
        raise NotImplementedError('TODO: implement platforms other than Windows for autodetection!')
        ...
        for l in libraries:
            for path in l.glob('appmanifest_*.acf'):
                ...
                if not app.path.exists():
                    logging.warn(f'Skipping steam app {app.appid} as ACF reported as installed but we cannot locate the folder')
                    continue
                apps.append(app)
        self.apps = apps
