...
STEAMRT_IMAGE_URL = STEAMRT_REPO_URL + '/com.valvesoftware.SteamRuntime.Sdk-amd64,i386-soldier-sysroot.tar.gz'
STEAMRT_IMAGE_SHA256 = '4a77fbd1ba45286eedf0446ecfd7d42ab0cc53bc1bc0cecd53efd75ba040598f'

class BaseCompileEnvironment():
	"""
	Base compilation environment
	"""
	def __init__(self, env: BuildEnvironment, config: LazyDynamicDotMap):
		self._env = env
		self._config = config
...
class NativeCompileEnvironment(BaseCompileEnvironment):
	...
	def run(self, srcdir: str, args: List[str]) -> int:
		...
		return self._env.run_tool(args, env=os.environ.copy().update(self._env_vars), cwd=cwd).returncode
...
class ChrootCompileEnvironment(BaseCompileEnvironment):
	...
	def run(self, srcdir: str, args: List[str]) -> int:
		...
		return self._env.run_subprocess(defargs.extend(args), env=os.environ.copy().update(self._env_vars), cwd=cwd).returncode
...
class DockerCompileEnvironment(BaseCompileEnvironment):
	...
	def _ensure_installed(self) -> bool:
		...
		rebuild_image = False
		ret = self._env.run_subprocess(['docker', 'inspect', '--type=image', DOCKER_IMAGE_TAG], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
		if ret.returncode != 0:
			rebuild_image = True
		...
		if sysroot_file.exists():
			...
			if not sysroot_hash == STEAMRT_IMAGE_SHA256:
				logging.warning('SteamRT docker image hash does not match, redownloading!')
				sysroot_file.unlink()
		...
		if rebuild_image:
			shutil.copyfile(Path(__file__).parent.joinpath('chaos-cas-steamrt.Dockerfile'), cache_folder.joinpath('Dockerfile'))
			ret = self._env.run_subprocess(['docker', 'build', '--tag', DOCKER_IMAGE_TAG, '.'], cwd=cache_folder)
	...
	def run(self, srcdir: str, args: List[str]) -> int:
		...
		defargs = ['docker', 'run', '--rm', '-i', '-v', f'{str(self._env.config.path.root)}:/app', '-w', f'/app/src/{srcdir}']
...
class PosixCompiler(BaseCompiler):
	...
	def __init__(self, env: BuildEnvironment, config: dict):
		...
		self._compile_env = _compile_environments[self._config.environment.type](env, self._config)
		self._dependencies = { BaseDependency(k, v) for k, v in self._config.dependencies.items() }
